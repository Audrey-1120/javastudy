...
public class MainClass {
  ...
  /*
   * java.io.File
   * 1. 디렉토리, 파일을 관리하는 클래스이다.
   * 2. 디렉토리, 파일을 만들고 삭제할 수 있다.
   ...
   * 경로 작성 방법
   * 1. 경로 구분자 (separator)
   *    1) 윈도우      : 역슬래쉬 (\\) --- 배포를 할때, 윈도우 기반 환경은 드물다 -> 이는 나중에 변경해야할 사항들(역슬래시-> 슬래시)이 많아질 수도 있다는 뜻.
   *    2) 리눅스, Mac : 슬래쉬 (/)
   *    3) File.separator 필드에는 플랫폼에 따른 경로 구분자가 자동 삽입된다.
   * 2. 디렉터리
  ...
  public static void method1() {
    ...
    // File 객체 생성
    dir = new File("\\storage"); // \\는 C드라이브, storage 는 폴더 이름.
    
    // 디렉터리 유무에 따른 생성/삭제 
    if(!dir.exists()) { // 디렉터리가 없으면...
      
      /*
       * 디렉토리가 없는지 확인하는 방법 -> 선택지가 2개임.
       * 1. not : ! --> (!dir.exists())
       * 2. == false --> (dir.exists() == false)
       */
...
//      dir.deleteOnExit(); // JVM 실행이 종료된 이후에 삭제한다.
  ...
  public static void method2() {
    ...
    // 파일을 File 객체로 생성 -> 디렉터리도 File 객체로 생성함.
